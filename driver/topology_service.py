...
class TopologyService(object):
    ...
    def on_config_map_update_event(self, event):
        ...
        if event_type in ['ADDED', 'MODIFIED']:
            ...
            if event_type != 'ADDED':
                logger.info('ConfigMap %s was %s' % (CSI_CONFIG_MAP_NAME, event_type.lower()))
            ...
            logger.debug('ConfigMap %s event %s: %s' % (CSI_CONFIG_MAP_NAME, event_type.lower(), event))

    def watch_topology_config_map(self):
        logger.info('Listening for changes on ConfigMap %s' % CSI_CONFIG_MAP_NAME)
        backoff = BackoffDelay(initial_delay=5, factor=2, max_delay=60)
        while not self.stop_event.is_set():
            try:
                stream, self.config_map_event_watcher = config_map_api.watch(CSI_CONFIG_MAP_NAME, timeout_seconds=10)
                ...
                logger.error('{} ConfigMap watcher encountered an error, will retry in {} seconds. Error: {}'.format(
                    CSI_CONFIG_MAP_NAME, backoff.current_delay, ex))
                backoff.wait()
